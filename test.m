...
load("./data/b12.mat");
c1 = size(Bit_con_tx1,2)*0.2;
test_Bit_con_tx1 = Bit_con_tx1(:,size(Bit_con_tx1,2)-c1+1:size(Bit_con_tx1,2));
test_Bit_con_tx2 = Bit_con_tx2(:,size(Bit_con_tx2,2)-c1+1:size(Bit_con_tx2,2));

c2 = size(Bit_index_tx,2)*0.2;
test_Bit_index_tx = Bit_index_tx(:,size(Bit_index_tx,2)-c2+1:size(Bit_index_tx,2));

c3 = size(noise,2)*0.2;
test_noise = noise(:,size(noise,2)-c3+1:size(noise,2));
...
Nsym = 2000; % number of OFDM blocks
